...
public class TestComponents {
    ...
    public void test() {
        ...
        Graph g = new Graph();
        Assert.assertEquals(new Components().getComponents(g).size(), 0);

        // returns singleton lists for unconnected nodes
        g.setNode("a");
        g.setNode("b");

        List<Graph> graphList = new Components().getComponents(g);
        ...
        graphList = new Components().getComponents(g);
        Assert.assertEquals(graphList.size(), 2);
        Assert.assertEquals(graphList.get(0).getNodes().toArray(), new String[]{"a", "b", "c", "d"});
        Assert.assertEquals(graphList.get(1).getNodes().toArray(), new String[]{"e", "f"});
